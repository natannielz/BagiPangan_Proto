                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="space-y-5">
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-600 focus:ring-brand-600">{{ old('description', $category->description) }}</textarea>
                        ...
                        <div>
                            ...
                            <select id="is_active" name="is_active" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-600 focus:ring-brand-600" required>
                                <option value="1" @selected((string) old('is_active', (int) $category->is_active) === '1')>Aktif</option>
                                <option value="0" @selected((string) old('is_active', (int) $category->is_active) === '0')>Nonaktif</option>
                            </select>
